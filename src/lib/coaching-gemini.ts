...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (/会議[\s\S]{0,48}提案[\s\S]{0,48}最後まで聞かず[\s\S]{0,32}却下[\s\S]{0,48}腹が立[\s\S]{0,64}準備時間[\s\S]{0,32}軽く扱[\s\S]{0,24}嫌/.test(text)) {
    return '【内部応答形式】提案を最後まで聞かずに却下され、準備時間を軽く扱われたことが嫌で腹が立ったという、本人が述べた事実と感情だけを一文で拾ってください。「お気持ちが伝わります」などAIの共感姿勢を足さず、次回の会議で相手に変えてほしい具体的な行動を質問一つだけで尋ねてください。';
  }
  ...
  if (/新しい役割[^。！？?\n]{0,32}引き受ける[^。！？?\n]{0,24}迷/.test(text)) {
    return '【内部応答形式】新しい役割を引き受けるか迷っているという事実を自然な日本語で一文だけ拾い、「その新しい役割を引き受ける判断で、一番引っかかっていることは何ですか」と質問一つだけで尋ねてください。「引き受けるか一番迷っている点」のように助詞を欠いた表現は使わないでください。';
  }
  ...
  if (/説明する順番[^。！？?\n]{0,24}迷/.test(text)) {
    return '【内部応答形式】説明順に迷っている事実を短く拾い、資料を最初に見る人へ伝えたい情報を一つだけ尋ねてください。「課題か機能か」のような二択、複数の回答対象、行動提案は付けないでください。';
  }
...
function buildConversationContinuityHint(
  ...
  if (tentativeAgreement) {
    ...
    if (/反応[^。！？?\n]{0,40}(?:心配|気にな|怖)/.test(previousAssistant)) {
      instructions.push(
        '- 直前が相手の反応への心配を尋ねる質問なら、その相手から言われると一番怖い一つの言葉を尋ねる。過去に実際に言われたとは仮定しない。'
      );
    } else {
      instructions.push(
