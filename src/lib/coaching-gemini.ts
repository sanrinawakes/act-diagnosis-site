...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (requestsSingleAnswerFormat(text) && /明日の朝/.test(text)) {
    return '【内部応答形式】ユーザーが指定した時機を保ち、回答を必ず「明日の朝、」で始めてください。「明日」だけに弱めたり、「その日」「翌日」に言い換えたりせず、ユーザーの「明日」をそのまま残してください。答えまたは提案は一動作だけ、一段落で簡潔に返してください。対象の作業名が会話にない場合は、「最初に行う手順」のような抽象語を作らず、「最初に取り組む具体的な作業名を一つメモに書く」という語順のまま、「〜してください」と書き、「最初に取り組み始める」のように開始を重ねないでください。Bot自身の予定に見える「〜します」で終えないでください。履歴にないスマートフォン、紙、場所、時刻、作業名を足さず、補足説明や質問も付けないでください。';
  }
