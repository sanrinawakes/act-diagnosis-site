...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (/条件[^。！？?\n]{0,24}(?:一覧|比較)[^。！？?\n]{0,40}(?:迷う理由|なぜ迷う|理由)[^。！？?\n]{0,24}(?:話したい|整理したい)/.test(text)) {
    return '【内部応答形式】ユーザーは条件比較ではなく、迷う理由を話したいと明示しています。直前の「何が引っかかっているか」を言い換えて聞き直さず、その役割を引き受けると想像した時に最初に頭へ浮かぶ具体的な場面を質問一つだけで尋ねてください。「整理していきましょう」などの進行宣言や一般論は付けないでください。';
  }
  ...
  if (isFactualLifeChangeWithoutExplicitRequest(text)) {
    return '【内部応答形式】これは人間関係または生活上の変化について、本人が事実だけを追加した発言です。書かれた事実を一文で短く受け止めた後、本人がその変化をどう感じているかを具体的な質問一つだけで尋ねてください。「どんな気持ちを感じていますか」という重複表現を使わず、「どのように感じていますか」と自然に尋ねてください。二択、解決策、負担や感情の先回りは付けないでください。';
  }
