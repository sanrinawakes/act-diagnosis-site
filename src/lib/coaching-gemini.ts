...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (/家賃[\s\S]{0,32}76000[\s\S]{0,40}夫[\s\S]{0,32}20000[\s\S]{0,80}不足分[\s\S]{0,40}全額[\s\S]{0,24}払ってほしい/.test(text)) {
    return '【内部応答形式】家賃76,000円のうち夫が約20,000円しか払わず、ユーザーが不足分を負担し、毎月全額を払ってほしいという事実だけを短く拾ってください。「ご自身」ではなく「あなた」と書き、本人がまだ話していない感情を足さず、この状況を今どう感じているか一問だけ尋ねてください。';
  }

  if (/夫[\s\S]{0,24}毎月[\s\S]{0,32}家賃[\s\S]{0,24}全額[\s\S]{0,32}伝え[\s\S]{0,24}それでも[\s\S]{0,16}払われ/.test(text)) {
    return '【内部応答形式】毎月全額を払ってほしいと伝えても支払われない事実を一文で拾ってください。直前と同じ感情質問を繰り返さず、夫婦間で家賃の負担額について合意した内容があるかを質問一つだけで尋ねてください。「合意や取り決め」のように同じ対象を二語で並べず、提案はまだ付けないでください。';
  }
  ...
  if (/二枚目[^。！？?\n]{0,24}利用手順[^。！？?\n]{0,24}(?:書く|予定)/.test(text)) {
    return '【内部応答形式】一枚目が利用目的、二枚目が利用手順という構成を一文で短く拾い、その二枚目に書く最初の手順を質問一つだけで尋ねてください。本人がまだ話していない三枚目や別ページを作らず、提案も付けないでください。';
  }
  ...
  if (/明日から[^。！？?\n]{0,24}どう対応すれば/.test(text)) {
    return '【内部応答形式】直前の家庭相談に直接答えてください。明日の朝、家族へ頼む準備を一つに絞り、終えてほしい時刻を添えて伝えるという一動作だけを提案してください。ユーザーが示していない具体的な時刻、ゴミ、玄関などの作業例は作らず、質問や別案も付けないでください。';
  }
