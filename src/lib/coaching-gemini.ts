...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (/夫[^。！？?\n]{0,24}家事[^。！？?\n]{0,24}後回し[\s\S]{0,80}負担[^。！？?\n]{0,24}腹が立/.test(text)) {
    return '【内部応答形式】家事を後回しにされ、本人ばかり負担していると感じて腹が立つという事実と感情をそのまま短く拾ってください。夫に家事を頼んだ時、いつやるかという返事があるかを尋ねる質問一つだけで返してください。本人がまだ話していない心理や理由は足さないでください。';
  }

  if (/条件[^。！？?\n]{0,24}(?:一覧|比較)[^。！？?\n]{0,40}(?:迷う理由|なぜ迷う|理由)[^。！？?\n]{0,24}(?:話したい|整理したい)/.test(text)) {
    return '【内部応答形式】ユーザーは条件比較ではなく、迷う理由を話したいと明示しています。「整理していきましょう」などの進行宣言や一般論を付けず、新しい役割について本人の中で一番引っかかっている具体的な一点を尋ねる質問一つだけを返してください。';
  }
  ...
  if (requestsSingleAnswerFormat(text)) {
    return '【内部応答形式】ユーザーの指定を優先し、答えまたは提案を一つだけ、一段落で簡潔に返してください。一つの提案へ二つの動作を「〜して、〜する」と詰め込まず、「AまたはB」「AやB」のような別案も付けないでください。対象の作業名が会話にない場合は「最初に行う手順」のような抽象語を作らず、「最初に取り組む具体的な作業名を一つメモに書く」という一動作にしてください。ユーザーが「明日」と指定した時は、「その日」「翌日」に言い換えず「明日」をそのまま残してください。補足説明や確認質問は付けず、答えた時点で終了してください。';
  }
