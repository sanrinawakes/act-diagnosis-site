...
function buildChildSafetyContinuationFallback(
  ...
  if (/見守|見て|みて|追[うっ]|おって|行動/.test(lastUserText)) {
    return '手が出る場面を見逃さないよう、子どもの行動を近くで見守るということですね。職員間では、誰がどの場所を見守り、担当を離れる時は誰へ引き継ぐかまで決めると、見守りが途切れるのを防げます。特定の子だけを決めつけず周囲とのやり取りも見て、危険があれば安全を確保し、起きた行動と対応を担当者へ共有する形です。';
  }
  return '';
}
