...
export const COACHING_RESPONSE_SPEED_INSTRUCTION = [
  ...
  '- 本人が書いていない「成果を出したい」「悲しい」、存在未確認の資料・指示書・未完了タスクを足さない。相手に遮られた事実を「共有できなかった」と本人側の不足へ変えない。',
  ...
  '- 同じ名詞や優先順位の語を一文の中で必要なく重ねない。「夫に家事を頼んだ時、夫から」「利用手順のページで、最初に行う手順」「最初に、まず一番」のような反復を避ける。',
  '- 「困ってしまいます」「心配になります」「聞くほど相手が黙ります」のような、本人が書いていない感情や因果を共感として補わない。',
  '- 本人が明言した感情を、近い別の感情へ言い換えたり追加したりしない。腹が立ったという発言へ、悔しさ、悲しさ、納得のいかなさを足さない。「当然です」「無理もありません」のような評価も足さない。',
  '- 「気持ちを受け止めます」「状況を受け止めます」のようにAI側の姿勢を宣言しない。本人が話した感情や事実を一文でそのまま拾う。',
  '- 「夫が理由を説明しない」に続く「何も言わない」は、夫が何も言わないという補足として扱う。AIが直前に尋ねた本人の感情への回答や、未実施の質問を夫へ試した結果に変えない。',
  ...
  '- 本人の「嫌」を「嫌悪感」へ強めない。一つの質問で「約束やルール」のように二つの回答対象を並べない。',
  '- 具体的な提案の前に「方法があります」「提案があります」と予告しない。実行する一文を直接示す。',
  '- 家賃や生活費の相談で、合意や契約条件を確認せず、相手名義の口座へ引落先を変える、自分の補填や支払いを止めるといった高影響の手続きを提案しない。記録と合意の確認、必要なら第三者が同席する公的・専門窓口へ進める。',
...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (/3回前[^。！？?\n]{0,40}締切時刻[^。！？?\n]{0,40}今日[^。！？?\n]{0,24}最初に直す箇所/.test(text)) {
    return '【内部応答形式】3回前を含む履歴から締切時刻を読み、「火曜午前10時の期限に向けて、今日最初に直す箇所は、一枚目の利用目的の長い説明です。」という構文で一文だけ返してください。「一枚目に書いている利用目的の長くなっている文章」のように修飾を重ねず、質問や別案も付けないでください。';
  }

  if (requestsFactualShortAnswer(text)) {
    return '【内部応答形式】会話履歴に保存されている事実から、ユーザーが尋ねた名前・日付・時刻・金額などの答えだけを「○○です」のように一文で返してください。日付だけを求められ、答えが25日なら「25日です」とだけ返し、「毎月の支払い日は」を繰り返さないでください。「大切にしている」などの質問文の言い換え、背景説明、追加質問は付けないでください。';
  }

  if (/友人[^。！？?\n]{0,32}断りたい予定[^。！？?\n]{0,40}返事[^。！？?\n]{0,24}先延ばし/.test(text)) {
    return '【内部応答形式】出力は「友人に断りたい予定があるのに、返事を先延ばしにしているのですね。\n\n返事を送ろうとした時、何が一番気になりますか？」の二文だけにしてください。「断りたい友人からの予定」のように、予定の向きを変えないでください。ユーザーは断り文句を求めていません。返事を送ろうとした時に何が一番気になるかを尋ね、引用文、断り方、本人が話していない友人の反応は先回りしないでください。';
  }

  if (/断る[^。！？?\n]{0,24}関係[^。！？?\n]{0,24}悪くなる[^。！？?\n]{0,24}気が/.test(text)) {
    return '【内部応答形式】断ると友人との関係が悪くなる気がするという本人の言葉だけを短く拾ってください。断りの文面や理由をユーザーへ作らせず、その友人からどのような反応をされるのが一番心配かを質問一つだけで尋ねてください。過去に同じ出来事があったとは仮定しないでください。';
  }

  if (/新しい仕事[\s\S]{0,48}失敗[\s\S]{0,48}期待を裏切る[\s\S]{0,32}怖[\s\S]{0,32}手をつけられ/.test(text)) {
    return '【内部応答形式】「新しい仕事を任され、失敗して期待を裏切るのが怖くて、手をつけられないのですね」と自然な一文で拾ってください。「新しい仕事を任された中で」「手をつけるのが止まっている」のように不自然に言い換えず、「その仕事で、特に失敗が気になる作業は何ですか」と質問一つだけで尋ねてください。';
  }

  if (/怖いというより[^。！？?\n]{0,32}同僚[^。！？?\n]{0,32}能力がない[^。！？?\n]{0,32}悔し/.test(text)) {
    return '【内部応答形式】「同僚に能力がないと思われるのが悔しいのですね」と本人の訂正を一文で拾ってください。直前と同じ「失敗が気になる具体的な作業」を言い換えて聞き直さず、「今回の仕事で、同僚にどの行動を見てほしいですか」と質問一つだけで会話を進めてください。本人が話していない誇り、価値、意欲は足さないでください。';
  }

  if (/会議[\s\S]{0,48}提案[\s\S]{0,48}最後まで聞かず[\s\S]{0,32}却下[\s\S]{0,48}腹が立[\s\S]{0,64}準備時間[\s\S]{0,32}軽く扱[\s\S]{0,24}嫌/.test(text)) {
    return '【内部応答形式】提案を最後まで聞かずに却下され、準備時間を軽く扱われたことが嫌で腹が立ったという、本人が述べた事実と二つの感情を一文で拾ってください。本人が明言した「嫌」を省かず、「お気持ちが伝わります」などAIの共感姿勢も足さず、次回の会議で相手に変えてほしい具体的な行動を質問一つだけで尋ねてください。';
  }

  if (/家賃[\s\S]{0,32}76000[\s\S]{0,40}夫[\s\S]{0,32}20000[\s\S]{0,80}不足分[\s\S]{0,40}全額[\s\S]{0,24}払ってほしい/.test(text)) {
    return '【内部応答形式】「家賃76,000円のうち夫が約20,000円しか払わず、あなたが不足分を負担していて、毎月全額払ってほしいのですね」と、金額・負担・本人の希望を省かず一文で拾ってください。「ご自身」ではなく「あなた」と書き、本人がまだ話していない感情を足さず、「この状況を今どのように感じていますか？」と一問だけ尋ねてください。';
  }

  if (/夫[\s\S]{0,24}毎月[\s\S]{0,32}家賃[\s\S]{0,24}全額[\s\S]{0,32}伝え[\s\S]{0,24}それでも[\s\S]{0,16}払われ/.test(text)) {
    return '【内部応答形式】「毎月全額払ってほしいと伝えても、支払われないのですね」と事実を一文で拾い、「夫婦間で家賃の負担額について合意した内容はありますか？」と質問一つだけで尋ねてください。「決めた取り決め」のように同じ意味を重ねず、直前と同じ感情質問や提案も付けないでください。';
  }

  if (/夫が家賃を払わない理由[\s\S]{0,32}何度聞いても[\s\S]{0,24}説明がありません/.test(text)) {
    return '【内部応答形式】「夫に家賃を払わない理由を何度聞いても、説明がないのですね」と、助詞を「夫に」に直して自然に一文で拾ってください。「夫が何度聞いても」のように、尋ねた人を夫へ入れ替えないでください。続けて「理由が分からないままの状況を、どのように感じていますか？」と質問一つだけで尋ねてください。';
  }

  if (/その伝え方[\s\S]{0,32}毎月[\s\S]{0,48}同じ提案[\s\S]{0,24}同じ質問/.test(text)) {
    return '【内部応答形式】同じ質問を重ねたことを一文で謝り、今回は「家賃の支払日、夫が支払った金額、あなたが補填した金額を、月ごとに一つの記録に残してください」と具体的に答えてください。「記録」を「メモ」「ノート」「表」「アプリ」「紙」へ言い換えず、夫へ再度伝える提案、相談窓口、第三者、別案、追加質問も付けないでください。';
  }

  if (/わからないから聞いて[\s\S]{0,32}質問を返さず[\s\S]{0,48}今までと違う対応/.test(text)) {
    return '【内部応答形式】直前までの履歴を読み、質問を返さず、前のターンで示した記録とは別の対応を一つだけ答えてください。家賃分担の相談では、自治体の「無料法律相談」を一件予約し、記録を持参する対応に絞ってください。「無料の法律相談」と言い換えず、女性相談窓口や夫婦問題窓口など別の相談先を並べず、「方法があります」と予告せず、追加質問も付けないでください。';
  }

  if (/明日また[^。！？?\n]{0,32}急な依頼[^。！？?\n]{0,48}角を立てず[^。！？?\n]{0,32}断る一言/.test(text)) {
    return '【内部応答形式】「ありがとうございます。ただ、今は手元の仕事に集中したいため、今回はお引き受けできません。」と、そのまま使える一文だけを返してください。本人が話していない今日の予定、明日までの猶予、別の引受時期を作らず、質問や補足も付けないでください。';
  }

  if (/次の会議[^。！？?\n]{0,40}責めず[^。！？?\n]{0,40}最初の一言/.test(text)) {
    return '【内部応答形式】「時間をかけて準備した提案ですので、今回は最後まで聞いてから判断してほしいです。」と、そのまま使える一文だけを返してください。本人の「嫌」「腹が立った」という感情を理由節へ入れず、補足説明や質問も付けないでください。';
  }

  if (requestsDirectWording(text)) {
    return '【内部応答形式】直近の会話を読み直し、ユーザーが明言した具体的な事実・感情・希望を少なくとも一つ含めて、そのまま読める一文を「」で一つだけ返してください。相手へ行動の変更を頼む文面は、本人の「嫌」という感情を理由節へ入れず、相手に変えてほしい具体的な行動を一つ入れてください。家事を後回しにされる相談なら、理由は「自分の時間を軽く扱われたように感じるので」と書き、お願いは「いつやるか教えてほしい」としてください。ユーザーが示していない時刻、時間、期限、事情は足さないでください。「少し話したいことがある」「今いいですか」のような許可取りだけの一般的な文、補足説明、追加質問は付けないでください。';
  }

  if (/夫[^。！？?\n]{0,24}家事[^。！？?\n]{0,24}後回し[\s\S]{0,80}負担[^。！？?\n]{0,24}腹が立/.test(text)) {
    return '【内部応答形式】家事を後回しにされ、本人ばかり負担していると感じて腹が立つという事実と感情をそのまま短く拾ってください。「ご自身」は使わず「あなた」または本人の言葉で書いてください。夫に家事を頼んだ時、いつやるかという返事があるかを尋ねる質問一つだけで返してください。本人がまだ話していない心理や理由は足さないでください。';
  }

  if (/条件[^。！？?\n]{0,24}(?:一覧|比較)[^。！？?\n]{0,40}(?:迷う理由|なぜ迷う|理由)[^。！？?\n]{0,24}(?:話したい|整理したい)/.test(text)) {
    return '【内部応答形式】ユーザーは条件比較ではなく、迷う理由を話したいと明示しています。直前の「何が引っかかっているか」を言い換えて聞き直さず、「その役割を引き受けると想像した時、最初に頭へ浮かぶのはどの場面ですか」と質問一つだけで尋ねてください。「具体的な場面はどんな場面ですか」のように同じ語を重ねず、「整理していきましょう」などの進行宣言や一般論も付けないでください。';
  }

  if (/新しい役割[^。！？?\n]{0,32}引き受ける[^。！？?\n]{0,24}迷/.test(text)) {
    return '【内部応答形式】新しい役割を引き受けるか迷っているという事実を自然な日本語で一文だけ拾い、「その新しい役割を引き受ける判断で、一番引っかかっていることは何ですか」と質問一つだけで尋ねてください。「迷われている」のような硬い敬語や、「引き受けるか一番迷っている点」のように助詞を欠いた表現は使わないでください。';
  }

  if (/(?:もう少し|もっと)[^。！？?\n]{0,16}深く聞いて/.test(text)) {
    return '【内部応答形式】直前の会話を読み、すでに尋ねた「何が引っかかるか」を言い換えて聞き直さないでください。「その新しい役割を引き受けても、あなたが守りたいものは何ですか」と、そのまま質問一つだけを返してください。「一番大切にしておきたいもの」のような弱い言い換えや、「手放したくないもののうち失われそうなもの」のように対象を重ねた表現を使わず、本人が話していない喪失や心理も断定しないでください。';
  }

  if (/時間[^。！？?\n]{0,24}軽く扱[^。！？?\n]{0,32}嫌/.test(text)) {
    return '【内部応答形式】ユーザーが述べた感情は「嫌」です。「家事そのものより、自分の時間を軽く扱われているように感じることが嫌なのですね」と、本人の感情を自然な一文でそのまま拾ってください。「ご自身」は使わず、「嫌悪感」「悲しさ」「悔しさ」へ言い換えないでください。引用文や伝え方はまだ提案せず、「夫に、まず変えてほしい行動は何ですか」と質問一つだけで尋ねてください。「約束やルール」のように二つの対象を並べないでください。';
  }

  if (/責める[^。！？?\n]{0,24}(?:言い方|伝え方)[^。！？?\n]{0,40}(?:喧嘩|けんか)[^。！？?\n]{0,40}落ち着いて伝えたい/.test(text)) {
    return '【内部応答形式】「責める言い方だと喧嘩になるので、落ち着いて伝えたいのですね」と本人の表現を強めずに一文で拾ってください。「だからこそ」「からこそ」のような強調を足さないでください。ユーザーはまだ一言や文面を求めていないため、引用文や伝え方を先回りして提案せず、「最初の一言には、夫へどんなお願いを入れたいですか」と質問一つだけで返してください。期限を決める提案や、すでに尋ねた「変えてほしい行動」の聞き直しはしないでください。';
  }

  if (/今日は[^。！？?\n]{0,24}一枚目[^。！？?\n]{0,24}直そう/.test(text)) {
    return '【内部応答形式】一枚目の利用目的から直す予定だと短く確認し、その利用目的の説明で最初に削りたい箇所を一つだけ尋ねてください。文章か箇条書きかの二択や別案を出さず、質問一つだけで返してください。';
  }

  if (/説明する順番[^。！？?\n]{0,24}迷/.test(text)) {
    return '【内部応答形式】出力は「説明する順番にも迷っているのですね。\n\n資料を初めて見る人へ、最初に伝えたい情報は何ですか？」の二文だけにしてください。資料を最初に見る人へ伝えたい情報を一つだけ尋ねます。「説明する順番でも」「迷われている」のような接客調を使わず、「説明資料の説明する順番」のように同じ語を重ねず、「最初」「まず」「一番」を同じ質問内で重ねる言い換えもしないでください。「課題か機能か」のような二択、複数の回答対象、行動提案も付けないでください。';
  }

  if (/二枚目[^。！？?\n]{0,24}利用手順[^。！？?\n]{0,24}(?:書く|予定)/.test(text)) {
    return '【内部応答形式】出力は「一枚目が利用目的、二枚目が利用手順という構成なのですね。\n\n二枚目に書く最初の手順は何ですか？」の二文だけにし、質問一つだけで返してください。「二枚目の利用手順で、利用者が最初に行う手順」のように「利用」と「手順」を重ねず、本人がまだ話していない三枚目や別ページを作らず、提案も付けないでください。';
  }

  if (/一枚目[^。！？?\n]{0,24}利用目的[^。！？?\n]{0,24}(?:書く|予定)/.test(text)) {
    return '【内部応答形式】「一枚目に利用目的を書く予定なのですね」と短く拾い、「その利用目的の次に、二枚目で伝えたい内容は何ですか」と質問一つだけで尋ねてください。「使い方かメリットか」のような二択や、本人がまだ決めていない内容の推測は付けないでください。';
  }

  if (/新しい企画[^。！？?\n]{0,24}説明資料[^。！？?\n]{0,24}作って/.test(text)) {
    return '【内部応答形式】「新しい企画の説明資料を作っているのですね」と本人の言葉を自然に拾ってください。「作成されている」のような接客敬語へ変えず、その資料を誰に向けて説明するのか、質問一つだけで尋ねてください。';
  }

  if (/対象[^。！？?\n]{0,24}初めて[^。！？?\n]{0,24}サービス[^。！？?\n]{0,24}使う人/.test(text)) {
    return '【内部応答形式】出力は「初めてサービスを使う人に向けた説明資料なのですね。\n\nその人に、資料で最初に伝えたいことは何ですか？」の二文だけにしてください。「最初」「まず」「一番」を同じ質問内で重ねず、本人が話していないサービスの特徴や行動も決めつけないでください。';
  }

  if (/目的[^。！？?\n]{0,24}説明[^。！？?\n]{0,24}長くなりすぎ/.test(text)) {
    return '【内部応答形式】出力は「利用目的の説明が長くなっているのですね。\n\nその説明で、必ず残したい内容は何ですか？」の二文だけにしてください。本人が示していない目標文字数を尋ねず、質問は核心として残す内容一つだけに絞ってください。';
  }

  if (/担当者[^。！？?\n]{0,24}資料[^。！？?\n]{0,24}火曜[^。！？?\n]{0,16}午前10時[^。！？?\n]{0,16}まで/.test(text)) {
    return '【内部応答形式】「火曜午前10時が資料の提出期限なのですね」と自然に一文で拾ってください。「提出の期限が示された」のような硬く間接的な表現へ変えず、「その期限までに、最初に終わらせる作業は何ですか？」と質問一つだけで尋ねてください。';
  }
  ...
  if (
    ...
    return '【内部応答形式】ユーザーは、実行方法と最後の質問の両方を明示的に求めています。まず、指定された時機に実行できる一動作を短く提案してください。企画書を明日始める相談なら、「明日、最初の15分で企画書の見出しを一つだけ書いてください」と具体的に答えてください。ユーザーが「明日」と指定した場合、提案文でも「明日」と書き、「その日」「翌日」へ言い換えないでください。「目次または見出し」のような二択を作らず、実行対象も一つに絞ってください。最後は「15分後に何が書けていれば、着手は成功だと判断しますか？」のように、直前の目的を実行する判断に直接つながる質問一つだけで閉じてください。「ご自身」は使わず、完璧にしたい箇所や理由の分析へ話を戻さないでください。';
  }

  if (/明日から[^。！？?\n]{0,24}どう対応すれば/.test(text)) {
    return '【内部応答形式】直前の家庭相談に直接答え、「明日の朝、家族に頼む準備を一つだけに絞り、終えてほしい時刻を添えて伝えてください。」と、そのまま一文だけ返してください。Bot自身の予定に見える「伝えます」で終えず、ユーザーが示していない具体的な時刻、ゴミ、玄関などの作業例は作らず、質問や別案も付けないでください。';
  }

  if (/家族[^。！？?\n]{0,32}朝の準備[^。！？?\n]{0,40}返事だけ[^。！？?\n]{0,32}動いてくれ/.test(text)) {
    return '【内部応答形式】「家族に朝の準備を頼んでも、返事だけで動いてくれないのですね」と自然な一文で拾い、「状況が起きている」のような重複表現を使わないでください。そのことを本人がどう感じているか、「どのように感じていますか」と質問一つだけで尋ねてください。解決策はまだ付けないでください。';
  }

  if (/仕事[\s\S]{0,32}整理[\s\S]{0,32}でき[\s\S]{0,48}家での相談/.test(text)) {
    return '【内部応答形式】「仕事の整理ができたのですね」と一文で短く拾い、「家での相談で、今気になっている出来事を一つ教えてください」と質問一つだけで尋ねてください。「出来事や状況」のように二つの回答対象を並べず、前の仕事へ話を戻したり、家庭の問題を推測したりしないでください。';
  }

  if (/仕事[^。！？?\n]{0,24}締切[^。！？?\n]{0,24}重な[^。！？?\n]{0,32}優先順位[^。！？?\n]{0,24}決めたい/.test(text)) {
    return '【内部応答形式】「仕事の締切が重なり、優先順位を決めたいのですね」と自然な日本語で一文だけ拾ってください。「優先順位を決めたい状況ですね」「現在抱えている重なっている仕事」のような硬さや重複を使わず、最も締切日時が近い作業名を質問一つだけで尋ねてください。仕事の内容と各締切日を同時に答えさせないでください。';
  }

  if (/話す直前[\s\S]{0,32}(?:一つ|ひとつ|1つ)(?:だけ)?[\s\S]{0,24}教えて/.test(text)) {
    return '【内部応答形式】「夫へ話しかける直前に、直前の会話で決めた最初の一言を頭の中で一度読み返してください」と、一動作だけを答えてください。「最初の一言」を「という言葉」へ弱めず、スマートフォンや紙など未提示の媒体を足さず、「声に出さず復唱」のような不自然で重複した表現も使わず、質問は付けないでください。';
  }

  if (requestsSingleAnswerFormat(text) && /明日の朝/.test(text)) {
    return '【内部応答形式】対象の作業名が会話にない場合、出力は「明日の朝、最初に取り組む具体的な作業名を一つメモに書いてください。」の一文だけにしてください。「メモに書く行動から進めてください」「メモに書く行動から始めてください」「最初に行う手順」「最初に取り組み始める」のように言葉を足さないでください。「明日の朝」を「明日」「その日」「翌日」に弱めず、履歴にないスマートフォン、紙、場所、時刻、作業名、補足説明、質問も付けないでください。';
  }

  if (requestsSingleAnswerFormat(text)) {
    return '【内部応答形式】ユーザーの指定を優先し、答えまたは提案を一つだけ、一段落で簡潔に返してください。一つの提案へ二つの動作を「〜して、〜する」と詰め込まず、「AまたはB」「AやB」のような別案も付けないでください。対象の作業名が会話にない場合は「最初に行う手順」のような抽象語を作らず、「最初に取り組む具体的な作業名を一つメモに書く」という語順のまま、一動作にしてください。「最初に取り組み始める」のように開始を重ねないでください。ユーザーが「明日」と指定した時は、「その日」「翌日」に言い換えず「明日」をそのまま残してください。提案はユーザーへの行動として「〜してください」と書き、Bot自身の予定に見える「〜します」で終えないでください。履歴にないスマートフォン、紙、場所、時刻、作業名を足さないでください。補足説明や確認質問は付けず、答えた時点で終了してください。';
  }

  if (isFactualLifeChangeWithoutExplicitRequest(text)) {
    return '【内部応答形式】これは人間関係または生活上の変化について、本人が事実だけを追加した発言です。書かれた事実を一文で短く受け止めた後、本人がその変化をどう感じているかを具体的な質問一つだけで尋ね、質問文は「？」で閉じてください。「ご自身」「担当される」は使わず、ユーザーが書いた「私が担当しています」を自然な「朝食準備も担当しているのですね」と拾ってください。「どんな気持ちを感じていますか」という重複表現も使わず、「どのように感じていますか？」と自然に尋ねてください。二択、解決策、負担や感情の先回りは付けないでください。';
  }

  if (requestsRestWithoutQuestions(text)) {
    return '【内部応答形式】今は掘り下げず、ユーザーが書いた「疲れた」「何も考えたくない」だけを使い、二文以内で休んでよいと伝えてください。「疲れがたまっている」「頭と体」のように状態を広げず、別の提案、再開案内、質問は付けないでください。';
  }

  if (/途中[^。！？?\n]{0,24}感情的[^。！？?\n]{0,24}不安|感情的になりそう/.test(text)) {
    return '【内部応答形式】「話の途中で感情が強くなってきたら、夫に『5分だけ休憩したい』と伝えてください。」と、そのまま一文だけ返してください。「感情的になりそうになったら」のように「なりそう」を重ねず、伝えた後にその場を離れる、別の場所へ移動する、呼吸するなど二つ目の動作を足さないでください。質問は付けないでください。';
  }
...
function buildConversationContinuityHint(
  ...
  if (answersWithSilence) {
    ...
    instructions.push(
      ...
      '- 受け止めは「何度聞いても、夫から説明がないのですね」という一文をそのまま使い、「夫が何も言わない状態が続いている」「何も言わない状態のまま」のような重複表現へ言い換えない。',
      '- 「夫に、まず何について説明してほしいですか」と、次に相手へしてほしい具体的な行動を一つだけ尋ねる質問で返す。'
    );
  }
  if (tentativeAgreement) {
    instructions.push(
      '- 「そうかも」は直前の問いへの暫定的な同意として扱い、同じ出来事や反応を言い換えて再質問しない。',
      '- 暫定同意から、本人が話していない過去の出来事、相手の反応、関係悪化の意味を作らない。受け止めの要約や見立てを足さず、質問一つだけを返す。'
    );
    if (/反応[^。！？?\n]{0,40}(?:心配|気にな|怖)/.test(previousAssistant)) {
      instructions.push(
        '- 直前が相手の反応への心配を尋ねる質問なら、「その友人から言われると一番怖い言葉は何ですか」と一問だけ尋ねる。「怖い言葉はどのような言葉ですか」のように同じ語を重ねず、過去に実際に言われたとも仮定しない。'
      );
    } else {
      instructions.push(
        '- 直前が過去の出来事の有無を尋ねる質問なら、その時に相手が実際に言った一つの言葉を尋ねる質問で一段深める。'
      );
    }
  }
  ...
  if (asksCoachToAnswer) {
    instructions.push(
      ...
      '- 回答を「方法があります」と予告せず、相談先や別案を複数並べず、確認済みの事実に直接つながる対応を一つだけ示す。'
    );
  }
