...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (/(?:もう少し|もっと)[^。！？?\n]{0,16}深く聞いて/.test(text)) {
    return '【内部応答形式】直前の会話を読み、すでに尋ねた「何が引っかかるか」を言い換えて聞き直さないでください。「その新しい役割を引き受けても、あなたが守りたいものは何ですか」のように、直前の対象につながる質問一つだけを自然な日本語で返してください。「手放したくないもののうち失われそうなもの」のように対象を重ねず、本人が話していない喪失や心理も断定しないでください。';
  }
