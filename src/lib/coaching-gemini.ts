...
function buildChildSafetyContinuationFallback(
  ...
  const recentAssistantContext = historyMessages
    ...
    .map((message) => message.content)
    .join('\n');
  if (/時刻|日時/.test(lastUserText) && /不明|覚えていません|はっきり/.test(lastUserText)) {
    return '日時が確かではない箇所を「不明」とする方針ですね。推測で正確な時刻を埋めず、分かる範囲だけを記すことで、確かな事実と未確認の点を区別できます。出来事の前後関係を覚えていれば、それも時刻とは別に書いてください。後から記録などで確認できた場合は、追記した日と確認方法を添えると経緯が分かります。';
  }
  if (/記憶|覚えて|記録|書く|書き/.test(lastUserText)) {
    if (/直接見た事実と人から聞いた話/.test(recentAssistantContext)) {
      return 'これから書き始めるのですね。正確に思い出せない日時を埋める必要はありません。まず覚えている出来事を一件選び、誰が誰に何をしたかを書いてください。発言をそのまま覚えていない場合は、引用ではなく「こういう趣旨だった」と区別して記すと、記憶の確かさも報告先へ伝わります。';
    }
  ...
  if (/見守|見て|みて|追[うっ]|おって|行動/.test(lastUserText)) {
    if (/見守る対象と方法を説明/.test(recentAssistantContext)) {
      return '誰を注意して見るのかを補足してくれたのですね。見守る対象を決めるだけでなく、その子が周りの子とどのように関わっているかも確認する方針として共有できます。危険がある時はすぐ対応できる距離を保ち、職員の交代時にも見守りが続くよう、担当を引き継いでください。';
    }
    if (/誰がどの場所を見守り/.test(recentAssistantContext)) {
      return '見守る対象と方法を説明してくれているのですね。子どもの様子を近くで見て、手が出る前に職員が気づけるようにする意図だと理解しました。職員へ共有する際は、「近くでやり取りを見て、危険を感じたら介入し、自分だけでは対応できない時は担当者を呼ぶ」と、現場で取る対応まで具体的に伝えてください。';
    }
...
function buildWorkplaceConflictContinuationFallback(
  ...
  if (reportsPastResignations || (/退職|辞め|やめ/.test(previousAssistant) &&
      /多かった|(?:これまで|以前|過去|その時).{0,60}(?:ましたか|ですか)/.test(previousAssistant) &&
      !/したい|つもり|予定/.test(previousAssistant) &&
  ...
  if (/相談/.test(previousAssistant) &&
      /近々|今週|近日|早め/.test(lastUserText)) {
    const timing = /今週/.test(lastUserText) ? '今週' : /早め/.test(lastUserText) ? '早め' : '近いうち';
    return `相談するのは${timing}がよいということですね。職場でどのような対応ができるかと、困っている状況を責任者へ伝える時期は別です。対応の可否がまだ決まっていなくても、相談を始めたいというあなたの希望は伝えられます。相談の場では、仕事で困る具体的な場面を説明すると、責任者も対応を検討しやすくなります。\n\n責任者には、まずどの出来事を知ってほしいですか？`;
  }
  if (/勤務|シフト/.test(lastUserText) && /ずら|変更|調整/.test(lastUserText) && /相談/.test(lastUserText)) {
    return '勤務を調整できるか、責任者へ相談したいのですね。相手に直接働きかける以外に、勤務の組み方を相談する方法を考えていると理解しました。変更できるかどうかは職場の状況によりますが、困っている場面と希望する調整を分けて伝えることはできます。\n\n勤務を変えられるとしたら、どの時間帯や業務をずらしたいですか？';
  }
  if (/無視/.test(lastUserText) && /頼|助け/.test(lastUserText)) {
    return '無視される相手に、仕事で助けを頼まなければならないことがつらいのですね。相手が返事をしない理由は分かりませんが、必要なやり取りができず困っていることは、あなたの気持ちとは別に仕事上の問題として扱えます。すぐ相手に何かを伝える方法を決める前に、どの場面で負担が大きいのかを聞きたいです。\n\n最近、助けを頼む必要があったのは、どのような場面でしたか？';
  }
  return '';
}
