...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (/夫[\s\S]{0,24}毎月[\s\S]{0,32}家賃[\s\S]{0,24}全額[\s\S]{0,32}伝え[\s\S]{0,24}それでも[\s\S]{0,16}払われ/.test(text)) {
    return '【内部応答形式】毎月全額を払ってほしいと伝えても支払われない事実を一文で拾ってください。直前と同じ感情質問を繰り返さず、夫婦間で家賃の負担額について合意した内容があるかを質問一つだけで尋ねてください。提案はまだ付けないでください。';
  }
  ...
  if (/(?:もう少し|もっと)[^。！？?\n]{0,16}深く聞いて/.test(text)) {
    return '【内部応答形式】直前の会話を読み、すでに尋ねた「何が引っかかるか」を言い換えて聞き直さないでください。現在の選択で本人が守りたいもの、手放したくないもの、または後悔しそうなことのうち、直前の対象につながる一点を質問一つだけで尋ねてください。本人が話していない心理は断定しないでください。';
  }

  if (/時間[^。！？?\n]{0,24}軽く扱[^。！？?\n]{0,32}嫌/.test(text)) {
    return '【内部応答形式】ユーザーが述べた感情は「嫌」です。「嫌悪感」「悲しさ」「悔しさ」へ言い換えず、そのまま短く受け止めてください。引用文や伝え方はまだ提案せず、夫に変えてほしい具体的な行動を一つだけ尋ねてください。「約束やルール」のように二つの対象を並べないでください。';
  }
