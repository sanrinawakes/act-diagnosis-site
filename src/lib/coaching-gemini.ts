...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (/わからないから聞いて[\s\S]{0,32}質問を返さず[\s\S]{0,48}今までと違う対応/.test(text)) {
    return '【内部応答形式】直前までの履歴を読み、質問を返さず、前のターンで示した記録とは別の対応を一つだけ答えてください。家賃分担の相談では、自治体の無料法律相談を一件予約し、記録を持参する対応に絞ってください。女性相談窓口や夫婦問題窓口など別の相談先を並べず、「方法があります」と予告せず、追加質問も付けないでください。';
  }
  ...
  if (/話す直前[\s\S]{0,32}(?:一つ|ひとつ|1つ)(?:だけ)?[\s\S]{0,24}教えて/.test(text)) {
    return '【内部応答形式】直前の会話で決めた最初の一言を、夫へ話しかける直前に頭の中で一度読み返すという一動作だけを答えてください。スマートフォンや紙など未提示の媒体を足さず、「声に出さず復唱」のような不自然で重複した表現も使わず、質問は付けないでください。';
  }
