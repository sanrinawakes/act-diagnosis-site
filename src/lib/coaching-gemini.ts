...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (/時間[^。！？?\n]{0,24}軽く扱[^。！？?\n]{0,32}嫌/.test(text)) {
    return '【内部応答形式】ユーザーが述べた感情は「嫌」です。「嫌悪感」「悲しさ」「悔しさ」へ言い換えず、そのまま短く受け止めてください。「ご自身」は使わず、本人が使った「私の時間」または自然な「自分の時間」と書いてください。引用文や伝え方はまだ提案せず、夫に変えてほしい具体的な行動を一つだけ尋ねてください。「約束やルール」のように二つの対象を並べないでください。';
  }
