...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (/友人[^。！？?\n]{0,32}断りたい予定[^。！？?\n]{0,40}返事[^。！？?\n]{0,24}先延ばし/.test(text)) {
    return '【内部応答形式】ユーザーは断り文句を求めていません。断りたい予定への返事を先延ばしにしている事実を短く拾い、返事を送ろうとした時に何が一番気になるかを質問一つだけで尋ねてください。引用文、断り方、本人が話していない友人の反応は先回りしないでください。';
  }
  ...
  if (/説明する順番[^。！？?\n]{0,24}迷/.test(text)) {
    return '【内部応答形式】「説明する順番にも迷っているのですね」と、そのまま自然に短く拾ってください。「説明する順番でも」「迷われている」のような接客調へ言い換えないでください。資料を最初に見る人へ伝えたい情報を一つだけ尋ね、「説明資料の説明する順番」のように同じ語を重ねず、「課題か機能か」のような二択、複数の回答対象、行動提案も付けないでください。';
  }

  if (/二枚目[^。！？?\n]{0,24}利用手順[^。！？?\n]{0,24}(?:書く|予定)/.test(text)) {
    return '【内部応答形式】「一枚目が利用目的、二枚目が利用手順という構成なのですね」と一文で短く拾い、「二枚目に書く最初の手順は何ですか？」と質問一つだけで尋ねてください。「二枚目の利用手順で、利用者が最初に行う手順」のように「利用」と「手順」を重ねず、本人がまだ話していない三枚目や別ページを作らず、提案も付けないでください。';
  }
