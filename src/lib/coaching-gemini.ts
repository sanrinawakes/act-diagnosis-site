...
function buildResponseStyleHint(text: string, hasAttachments = false) {
  ...
  if (/新しい仕事[\s\S]{0,48}失敗[\s\S]{0,48}期待を裏切る[\s\S]{0,32}怖[\s\S]{0,32}手をつけられ/.test(text)) {
    return '【内部応答形式】ユーザーの「失敗して期待を裏切るのが怖くて、手をつけられない」という言葉を自然な日本語のまま短く拾ってください。「手をつけるのが止まっている」のように不自然に言い換えず、その新しい仕事で特に失敗が気になる具体的な作業を質問一つだけで尋ねてください。';
  }
  ...
  if (/仕事[^。！？?\n]{0,24}締切[^。！？?\n]{0,24}重な[^。！？?\n]{0,32}優先順位[^。！？?\n]{0,24}決めたい/.test(text)) {
    return '【内部応答形式】仕事の締切が重なり優先順位を決めたいという事実を、自然な日本語で一文だけ拾ってください。「現在抱えている重なっている仕事」のような重複表現を使わず、最も締切日時が近い作業名を質問一つだけで尋ねてください。仕事の内容と各締切日を同時に答えさせないでください。';
  }
  ...
  if (isFactualLifeChangeWithoutExplicitRequest(text)) {
    return '【内部応答形式】これは人間関係または生活上の変化について、本人が事実だけを追加した発言です。書かれた事実を一文で短く受け止めた後、本人がその変化をどう感じているかを具体的な質問一つだけで尋ねてください。「ご自身」「担当される」は使わず、ユーザーが書いた「私が担当しています」を自然な「朝食準備も担当しているのですね」と拾ってください。「どんな気持ちを感じていますか」という重複表現も使わず、「どのように感じていますか」と自然に尋ねてください。二択、解決策、負担や感情の先回りは付けないでください。';
  }
