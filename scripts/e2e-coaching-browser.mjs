...
try {
  ...
  const invalidOutputs = generatedCoachingOutputs.filter(({ content }) =>
    /お察し(?:いた)?します|承知(?:いた)?しました|ご安心ください|(?:お気持ち|気持ち).{0,8}よく(?:分|わ)かります|何か(?:具体的に|続けて)?(?:お話し|話して)(?:みたい|したい)?ことはありますか|(?:感情|気持ち|怖さ|不安|怒り|悲しさ).{0,16}(?:横|脇)に置|タタスク|タースク|タムスケジュール|(?:です|ます)[。．]\s*か[？?]|途中で止まることはありません|必ず(?:回答|返答)します/.test(
      content
    )
  );
  addCheck(
